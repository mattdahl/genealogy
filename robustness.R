...
## Calculate the total number of immediate children for each case
...
(function () {
  ...
  plot(
    density(XiDescendentCountProbs),
    main = caseTypesLabels[i],
    xlab = 'Posterior Parent Probability',
    ylab = 'Density',
    xlim = c(0,1)
  )
...
(function () {
  ...
  barplot(
    table(allChildren),
    main = 'Distribution of Total Number\nof Child Cases (Truncated)',
    xlab = 'Number of Estimated Child Cases',
    ylab = 'Density',
    xlim = c(0,14),
    col = 'grey'
  )
...
(function () {
  ...
  plot(
    c(0, 40),
    c(0, 0.15),
    main = 'Distribution of Parent\nAge at Childbirth',
    xlab = 'Age of Estimated Parent Case',
    ylab = 'Density',
    type = 'n'
  )
  ...
  lines(
    density(ageAtChildbirth, na.rm = TRUE),
    col = colors[1]
  )
...
(function () {
  ...
  plot(
    LiiXs,
    LiiByChildren[which(!is.na(LiiByChildren))],
    ylab = 'Legal Information Institute List Inclusion',
    xlab = 'Number of Child Cases',
  ...
  plot(
    OxfordXs,
    OxfordByChildren[which(!is.na(OxfordByChildren))],
    ylab = 'Oxford Important Case List Inclusion',
    xlab = 'Number of Child Cases',
  ...
  plot(
    NytXs,
    NytByChildren[which(!is.na(NytByChildren))],
    ylab = 'New York Times Coverage',
    xlab = 'Number of Child Cases',
...
(function () {
  ...
  plot(
    jitter(rescale(AllAuthority)),
    jitter(rescale(allChildren)),
    pch = 21,
    cex = 0.75,
    col = 'grey',
    bg = 'grey',
    xlab = 'Normalized Fowler and Jeon Authority Score',
    ylab = 'Normalized Number of Child Cases',
    main = 'Legal Authority and Fertility'
  )
  abline(lm(rescale(allChildren)~rescale(AllAuthority)))
  plot(
    jitter(rescale(AllHub)),
    jitter(rescale(allChildren)),
    pch = 21,
    cex = 0.75,
    col = 'grey',
    bg = 'grey',
    xlab = 'Normalized Fowler and Jeon Hub Score',
    ylab = 'Normalized Number of Child Cases',
